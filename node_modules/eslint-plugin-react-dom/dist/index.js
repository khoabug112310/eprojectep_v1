...
function create2(context) {
  ...
  return {
    JSXElement(node) {
      ...
      const initialScope = context.sourceCode.getScope(node);
      const hasChildren = node.children.some(isSignificantChildren) || ER__namespace.hasAttribute(context, "children", attributes, initialScope);
...
function isPaddingSpaces(node) {
  return ER__namespace.isJsxText(node) && isWhiteSpace(node) && node.raw.includes("\n");
}
function isSignificantChildren(node) {
  return node.type !== types.AST_NODE_TYPES.JSXText || !isPaddingSpaces(node);
}
